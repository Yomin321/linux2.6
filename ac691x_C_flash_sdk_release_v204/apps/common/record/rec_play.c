...
void recode_play_mutex_init(void *priv)
{
    if (priv) {
...
void recode_play_mutex_stop(void *priv)
{
    if (rec_play_hdl) {
        rec_play_exit();
        rec_play_hdl = NULL;
    }
    mutex_resource_release("recode_play");
}
...
void rec_play_api(REC_FILE_INFO *rec_file_info)
{
    ...
    mutex_resource_apply("recode_play", 3, recode_play_mutex_init, recode_play_mutex_stop, rec_file_info);
}

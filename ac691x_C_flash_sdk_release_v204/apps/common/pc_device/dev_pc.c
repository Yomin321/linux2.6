...
#include "usb/usb_slave_internal_api.h"
#include "sdk_cfg.h"
#include "ladc.h"
#include "msg.h"
#include "flash_api.h"
...
#define PC_INDEPEND_VOL     0   //独立音量

#define PC_ALLOC_POOL_SIZE  (5*1024 + 512)
static u8 pc_alloc_pool[PC_ALLOC_POOL_SIZE] sec_used(.pc_mem);
...
static u8 protect_buffer[PROTECT_BUFF_MAX] sec_used(.pc_mem);
...
const u8 USER_RIPRODUCT_STR[] = {
    0x22, 0x03,
    'J', 0, 'i', 0, 'e', 0, 'L', 0, 'i', 0, '-', 0,
    'B', 0, 'R', 0, '2', 0, '0', 0, ' ', 0, 's', 0, 'p', 0, 'k', 0, 'e', 0, 'r', 0
};
...
static u32 pc_output_cb(s16 *buffer, u32 buffer_len)
{
    ...
    /* } */
    /* return 0; */
    return dac_write(buffer, buffer_len);
}
...
*/
/*----------------------------------------------------------------------------*/
static void usb_slave_spk_output_io_reg(usb_slave_dev_io *dev_io)
{
    u32 parm;
    ...
    __this(dev_ioctrl)((void *)&parm, USB_SLAVE_SPK_OUTPUT_IO_REG);
}
/*----------------------------------------------------------------------------*/
/**@brief  PC MIC输入接口
...
#if PC_INDEPEND_VOL
    sound.tmp_sys_vol_l = dac_ctl.sys_vol_l;
    sound.tmp_sys_vol_r = dac_ctl.sys_vol_r;
//    dac_ctl.sys_vol_l = PC_DEFAULT_VOL;
//    dac_ctl.sys_vol_r = PC_DEFAULT_VOL;
#endif
...
#if PC_INDEPEND_VOL
    dac_ctl.sys_vol_l = sound.tmp_sys_vol_l;
    dac_ctl.sys_vol_r = sound.tmp_sys_vol_r;
#endif
...
s32 app_usb_slave_init(void)
{
    ...
    /* 注册SPK USB中断回调  */
    usb_slave_spk_output_io_reg(dev_io);
...
u8 app_pc_set_speaker_vol(u32 pc_mute_status)
{
    ...
    if (spk_vol_l) {
        spk_vol_l /= 0xff / get_max_sys_vol(0);
    ...
    spk_vol_r &= 0xff;
    if (spk_vol_r) {
        spk_vol_r /= 0xff / get_max_sys_vol(1);
    ...
    dac_ctl.sys_vol_r = spk_vol_r;
    dac_ctl.sys_vol_l = spk_vol_l;
    set_sys_vol(dac_ctl.sys_vol_l, dac_ctl.sys_vol_r, FADE_ON);
...
u32 app_usb_slave_online_status(void)
{
    ...
    return __this(dev_ioctrl)(&online_status, USB_SLAVE_GET_ONLINE_STATUS) ;  //USB_SLAVE_GET_ONLINE_STATUS)
}
/*----------------------------------------------------------------------------*/
/**@brief  PC 在线列表
...
void pc_check_api(void)
{
    ...
    dev_status = __this(dev_ioctrl)(&parm, USB_SLAVE_ONLINE_DETECT);

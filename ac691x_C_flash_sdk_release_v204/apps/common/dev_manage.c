...
#if 0
#define dev_dbg  otp_printf
#define mbr_deg  otp_printf
#else
#define dev_dbg(...)
#define mbr_deg(...)
#endif
...
int mbr_scan_parition(MBR_DRIVE_INFO *mbr_inf, DEV_HANDLE hdev, void *read_fun, void *write_fun)
{
    ...
    if (DEV_ERR_NONE != dev_io_ctrl(hdev, DEV_GET_BLOCK_SIZE, &parm)) {
        otp_printf("DEV_GET_BLOCK_SIZE\r");
#if MBR_MALLOC
        free(mbr);
#endif
        return MBR_FAIL;
    }
    otp_printf("DEV_GET_BLOCK_SIZE = %d\r", parm);
    parm /= 512;
...
#if 0//((USB_SD0_MULT_EN == 1)||(USB_SD1_MULT_EN == 1))
...
*/
/*----------------------------------------------------------------------------*/
#define MAX_DEV_REG				3		//注册的设备最大数
static u8 device_manage_need_buf[256] ALIGNED(4);
#define DET_IO_INIT()    do{JL_PORTA->DIR |= BIT(3);JL_PORTA->PD &= ~BIT(3);JL_PORTA->PU |= BIT(3);}while(0)
#define GET_IO_STATUS()    (JL_PORTA->IN & BIT(3))
bool io_det_func()
{
    DET_IO_INIT();
    if (0 == GET_IO_STATUS()) {
...
void dev_manage_init(void)
{
    ...
    sd0_api_set.controller_io = SD0_IO_A;        	//SD0_IO_A：SD0控制器B出口，SD0_IO_B：SD1控制器B出口
    ...
    sd0_api_set.io_det_func = io_det_func;               	//IO检测函数注册
    ...
    sd1_api_set.io_det_func = io_det_func;               	//IO检测函数注册
    ...
    while (1) {
        ...
        if (dev_get_online_status(dev, (void *)&parm) == DEV_ERR_NONE) {
            if (parm == DEV_ONLINE) {
                ...
                extern u32 fs_open_file_bypath(void *dev_hdl, void *buf, u16 len, char *path);
                fs_open_file_bypath(dev, buf, 512, "/my_test.txt");
                ret = dev_unmount(dev);
                ASSERT(!ret);

...
static u8 *rx_uart0_buf = NULL;
u8 *rx_uart1_buf = NULL;
static u8 *rx_uart2_buf = NULL;
...
void uart0_isr_fun()
{
    ...
    if (JL_UART0->CON0 & BIT(11)) {    //OTCNT PND
        ...
        if (uart_info[0].callback_fun) {
            uart_info[0].callback_fun(0, rx_uart0_buf, UART_ISR_TYPE_TIMEOUT);
        }
...
#error "EQ uart debug not config uart port IO"
    ...
    memset((u8 *)eq_uart_recv_buf, 0x0, 512);
    IRQ_REQUEST(IRQ_UART1_IDX, uart1_isr_fun);
    rx_uart1_buf = eq_uart_recv_buf;
    ut_dma_wr_cnt[1] = sizeof(eq_uart_recv_buf);
    /* otp_printf("dma_add %x %x cnt %d\n", &eq_uart_recv_buf[1], rx_uart0_buf, ut_dma_wr_cnt[1]); */
    JL_UART1->RXCNT = ut_dma_wr_cnt[1];
    JL_UART1->RXSADR = (u16)rx_uart1_buf;
    JL_UART1->RXEADR = (u16)(rx_uart1_buf + ut_dma_wr_cnt[1]);
    JL_UART1->OTCNT = 20000 * 1000;

    JL_UART1->CON0 =  BIT(13) | BIT(12) | BIT(10) | BIT(7) | BIT(6) | BIT(5) | BIT(3) | BIT(0);
    return 0;
}
#endif

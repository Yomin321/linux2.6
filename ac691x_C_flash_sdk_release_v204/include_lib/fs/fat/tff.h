...
struct _FS_NAME {
    ...
    u32    *lfn;             //长文件名buffer，长度LFN_MAX_SIZE×2，（前LFN_MAX_SIZE）为长文件名，（后LFN_MAX_SIZE）为长文件夹名
    u32    *ldn;             //长文件名buffer，长度LFN_MAX_SIZE×2，（前LFN_MAX_SIZE）为长文件名，（后LFN_MAX_SIZE）为长文件夹名
    u16     lfn_cnt;
    u16     ldn_cnt;
};
...
bool decode_lfn(u32 *target_p, u32 *source_p, u16 max_copy); //const char *file_type)
